...
export default async function Page() {
  ...
  try {
    ...
    const data = await prisma.customer.findMany({ include: { user: true }, orderBy: { createdAt: "desc" }, take: 50 });
    rows = data.map((c) => ({ id: c.id, summary: c.user.email, updatedAt: c.updatedAt }));
